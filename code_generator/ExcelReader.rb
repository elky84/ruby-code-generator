...
for row in 0...row_length
	...
	if option.include? "json:patchdata"      
		if row > 2
		  dir = "#{userprofile}\\AppData\\LocalLow\\ELKY\\Sunbee\\#{selected_sheet.rows[row][3]}"    
...
arr.each do |str|
	...
	if command.include? "json"
	  ...
	  if param == "patchdata" or param == "file"
	    loader_create = false
	  elsif param == "parser"
	  ...
	  if make_json_file == true
  	  ...
  	  dir = "#{userprofile}\\AppData\\LocalLow\\ELKY\\Sunbee\\JSON"
  	  if param == "patchdata"
  	    dir = "#{userprofile}\\AppData\\LocalLow\\ELKY\\Sunbee\\List"
  	  elsif param == "file"
        dir = destination
  	  end
